...
The `Evaluation\Admin\Grid\Column` object has a built-in `display()` method to handle the value of the current column through the incoming callback function:
```php
...
use Evaluation\Admin\Grid\Displayers\AbstractDisplayer;
